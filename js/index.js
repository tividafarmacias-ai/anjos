...
function openDetailsModal(id) {
  ...
  content.innerHTML = detailsFields
    .map((field) => {
      const value = field === "MATRIZ_FILIAL" ? record["MATRIZ_FILIAL"] : record[field];
      ...
      let rawValue = formatValue(value); // Valor sem tags HTML para o botão copiar
      let renderedValue = escapeHtml(rawValue);
      ...
      if (isWhatsapp && whatsappNumber) {
        ...
        `;
      } else if (isInstagram && instagramUsername) {
        renderedValue = `
      ...
      // Template atualizado com botão de cópia
      return `
        <div class="detail-card">
          <div class="detail-header">
            <h4>${escapeHtml(field)}</h4>
            <button class="copy-btn" onclick="copyToClipboard(this, '${escapeHtml(rawValue)}')" title="Copiar dado">
